...
<%--
$Workfile:   ObjectInfo.jsp  $
$Revision:   1.7  $
$Date:   20 Sep 2004 11:23:36  $
--%>
<%@ include file="/WEB-INF/jsp/WcmHeader.jsp" %>
<%@ page import="com.filenet.wcm.apps.server.ui.*, com.filenet.wcm.apps.server.util.*" %>
<%
Locale locale = WcmStringResources.getClientLocale(request);
WcmString wsTitle = new WcmString(locale, "server.properties_WcmMoreObject_jsp.title", "Object Properties");
%>
<html>
<head>
    <%
       WcmWorkplaceHeader header = (WcmWorkplaceHeader)        WcmWorkplaceUi.getHeaderModule(request);
       WcmWorkplaceUi.renderHeaders(request, out, wsTitle);
    %>
...
<body class="wcmBody">
<% 
	WcmUi.render(request, "objectInfo", out); 
%>
